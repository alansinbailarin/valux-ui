...
export function PopoverTrigger({
  ...
  const { open, phase, setOpen, triggerRef, popoverId } = usePopoverContext();
  ...
  const shared = {
    ...
    "aria-expanded": open,
    "aria-controls": hidden ? popoverId : undefined,
  ...
  if (asChild && isValidElement<ChildProps>(children)) {
    ...
    return cloneElement(
      ...
      // eslint-disable-next-line react-hooks/refs
      {
        ...props,
        ...shared,
        className: ["vx-popover-trigger", child.props.className, className]
          .filter(Boolean)
          .join(" "),
      ...
      <span className="vx-popover-trigger__content">{child.props.children}</span>,
    );
  }
  ...
  return (
    <button
      ...
      {...shared}
      type="button"
      className={["vx-popover-trigger", className].filter(Boolean).join(" ")}
      onClick={handleClick}
    >
      <span className="vx-popover-trigger__content">{children}</span>
    </button>
  );
}
